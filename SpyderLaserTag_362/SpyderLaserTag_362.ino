...
void loop() {
  ...
  if(radio.isChipConnected() && radio.available()) {
    ...
    //Check if command 5 is received. This will cause a change in state.
    if(command.equals("5")) {
      idle = !idle;
    }

    //Execute command if not idling
    if(!idle) {
      ...
      } else if (command.equals("3")) { //Move forward
         Serial.println("Foward");
         motorControl(LOW, HIGH, LOW, HIGH);
      } else if (command.equals("4")) { //Move backward
    ...
    }//End of checking idle state
  }//End of checking radio

  //Check if it is idling state
  if(idle) {
